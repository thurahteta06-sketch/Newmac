#!/usr/bin/env python3
# bot.py - Fast ARP reader (no ping sweep)
...
TOKEN = "8905971731:AAHcbRrLS9LtCfvpdDqzUR5KHplyB5JE5zg"
...
def read_arp_table() -> list[dict]:
    """Read /proc/net/arp and return list of devices with IP and MAC."""
    devices = []
    try:
        ...
        logging.error(f"ARP read error: {e}")
    return devices
...
async def auto_scan_from_url(portal_info: dict, update: Update):
    ...
    for i, dev in enumerate(devices):
        ...
        await asyncio.sleep(0.05)  # slight delay to avoid CPU hog
...
# ─────────────────────────────────────────
#  Commands (start, setup, saved, clear, handle_text)
# ─────────────────────────────────────────
...
def main():
    ...
    print("🤖 Bot စတင်နေပါပြီ... (Fast ARP mode)")
    app.run_polling()
